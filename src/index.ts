...
export default function laravel(config: string | string[] | PluginConfig): [LaravelPlugin, ...Plugin[]] {
  ...
  return [resolveLaravelPlugin(pluginConfig), ...(resolveFullReloadConfig(pluginConfig) as Plugin[])]
}
        ...
        if (isAddressInfo(address)) {
          ...
          setTimeout(() => {
            server.config.logger.info(
              `\n  ${colors.red(`${colors.bold('LARAVEL')} ${laravelVersion()}`)}  ${colors.dim(
                'plugin',
...
function laravelVersion(): string {
  try {
    ...
    return (
      composer.packages?.find((composerPackage: { name: string }) => composerPackage.name === 'laravel/framework')
        ?.version ?? ''
    )
  } catch {
    return ''
...
function resolveValetHost(): string {
  ...
  if (!fs.existsSync(configPath)) {
    throw Error(
      'Unable to find the Valet configuration file. You will need to manually specify the host in the `valetTls` configuration option.',
    )
  }
