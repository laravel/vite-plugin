...
describe('laravel-vite-plugin', () => {
  ...
  it('handles missing configuration', () => {
    ...
    /* @ts-ignore */
    expect(() => laravel()).toThrowError('laravel-vite-plugin: missing configuration.')

    /* eslint-disable-next-line @typescript-eslint/ban-ts-comment */
    /* @ts-ignore */
    expect(() => laravel({})).toThrowError('laravel-vite-plugin: missing configuration for "input".')
  })
  ...
  it('accepts an array of inputs', () => {
    const plugin = laravel(['resources/js/app.ts', 'resources/js/other.js'])[0]
  ...
  it('prevents setting an empty publicDirectory', () => {
    expect(() => laravel({ input: 'resources/js/app.js', publicDirectory: '' })[0]).toThrowError(
      'publicDirectory must be a subdirectory',
    )
  })

  it('prevents setting an empty buildDirectory', () => {
    expect(() => laravel({ input: 'resources/js/app.js', buildDirectory: '' })[0]).toThrowError(
      'buildDirectory must be a subdirectory',
    )
  })
